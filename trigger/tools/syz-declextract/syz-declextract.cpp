...
class DeclExtractCallVisitor : public RecursiveASTVisitor<DeclExtractCallVisitor>
{
	...
	bool VisitFunctionDecl(const FunctionDecl* D)
	{
		...
		for (auto prefix : ignore_prefixes) {
			if (strncmp(fn.c_str(), prefix, strlen(prefix)) == 0)
				return true;
		}
		...
		if (strstr(src.c_str(), "/um/") == 0)
			return true;
		for (auto file : ignore_files) {
			if (strstr(src.c_str(), file))
				return true;
		}
		for (const ParmVarDecl* P : D->parameters()) {
			auto typ = convertType(Context, P->getType());
			if (typ == "") {
				llvm::outs() << D->getNameInfo().getAsString() << ": UNKNOWN TYPE: " << QualType(P->getType()).getAsString() << "\n";
				return true;
			}
		}
