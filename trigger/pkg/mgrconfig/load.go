...
func Complete(cfg *Config) error {
	...
	var err error
	cfg.Priorities, err = ParseUserPriorities(cfg.UserPriorities)
	if err != nil {
		return err
	}
...
func ParseUserPriorities(user []string) (map[uint32]float64, error) {
	priorities := make(map[uint32]float64)
	var err error

	for _, prio := range user {
		u := strings.SplitN(prio, ":", 2)
		if len(u) != 2 {
			return nil, fmt.Errorf("unknown priority: %v", prio)
		}

		key, err := strconv.ParseUint(u[0], 10, 32)
		if err != nil {
			return nil, fmt.Errorf("unknown priority key: %v", u[0])
		}

		val, err := strconv.ParseFloat(u[1], 64)
		if err != nil {
			return nil, fmt.Errorf("unknown prority value: %v", u[1])
		}
		priorities[uint32(key)] = val
	}
