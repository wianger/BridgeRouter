...
bool isAllocFn(StringRef name, int *size, int *flag) {
  ...
  if (!name.compare("kmalloc_array") || !name.compare("kcalloc")) return false;
  ...
  if (!name.compare("kstrndup") || !name.compare("kstrdup")) return false;
  ...
  if (!name.compare("mempool_alloc_pages")) return false;
...
std::string getStructId(Value *PVal, User::op_iterator &IS,
  ...
  if (debug) fprintf(stderr, "debug mode in getStructId()\n");
  ...
  */
  ...
  if (STy && !STy->isOpaque() && !STy->isLiteral()) {
    ...
    */
    ...
    if (debug) fprintf(stderr, "struct id: %s\n", rso.str().c_str());
    return rso.str();
  }
  return "";
}
...
std::string getAnonStructId(Value *V, Module *M, StringRef Prefix) {
  ...
  while (!WorkList.empty()) {
    ...
    if (PVal) {
      ...
      if (!structId.empty()) return structId;
    }
...
std::string getAnnotation(Value *V, Module *M) {
  ...
  while (!WorkList.empty()) {
    ...
    if (GlobalVariable *GV = dyn_cast<GlobalVariable>(v)) return getVarId(GV);

    if (Argument *A = dyn_cast<Argument>(v)) return getArgId(A);
    ...
    if (PVal) {
      ...
      if (!structId.empty()) {
        ...
        if (Visited.insert(PVal).second) WorkList.push_back(PVal);
    ...
    if (CallInst *CI = dyn_cast<CallInst>(v)) {
      ...
      if (ConstantExpr *CE = dyn_cast<ConstantExpr>(CV)) {
        if (CE->isCast()) CV = CE->getOperand(0);
      }
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(v)) {
      ...
      if (Visited.insert(S).second) WorkList.push_back(S);
      continue;
    }
