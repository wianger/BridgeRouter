...
bool CallGraphPass::isCompatibleType(Type *T1, Type *T2) {
  if (T1->isPointerTy()) {
    ...
    if (ElT1->isIntegerTy(8) /*|| ElT2->isIntegerTy(8)*/) return true;
    ...
    if (ST1->isLiteral() != ST2->isLiteral()) return false;

    // literal, compare content
    if (ST1->isLiteral()) {
      unsigned numEl1 = ST1->getNumElements();
      if (numEl1 != ST2->getNumElements()) return false;
    ...
    if (numParam1 != FT2->getNumParams()) return false;
...
bool CallGraphPass::findCalleesByType(CallInst *CI, FuncSet &FS) {
  AbstractCallSite CS(&CI->getCalledOperandUse());  // FIXME: argument
  // errs() << *CI << "\n";
  for (Function *F : Ctx->AddressTakenFuncs) {
    ...
    } else if (F->arg_size() != CS.getNumArgOperands()) {
    ...
    for (Function::arg_iterator FI = F->arg_begin(), FE = F->arg_end();
      ...
      Type *ActualTy = CS.getCallArgOperand(AI)->getType();
...
bool CallGraphPass::runOnFunction(Function *F) {
  ...
  if (F->hasSection() && F->getSection().str() == ".init.text") return false;
  bool Changed = false;
  ...
  for (inst_iterator i = inst_begin(F), e = inst_end(F); i != e; ++i) {
    ...
    if (StoreInst *SI = dyn_cast<StoreInst>(I)) {
      ...
      if (!srcTy->isStructTy() || !dstTy->isPointerTy()) continue;

      dstTy = dstTy->getPointerElementType();
      if (!dstTy->isFunctionTy()) continue;
      ...
      if (srcTy->getStructName().equals("union.security_list_options")) {
        ...
        for (auto callee : FS) {
          if (!FuncName.contains(
                  callee->getName().substr(8)))  // get rid of "selinux_"
            continue;
          while (!dyn_cast<CallInst>(I)) I = I->getNextNode();
        ...
        if (Function *F = dyn_cast<Function>(CS->getOperand(i))) {
          ...
          if (!STy->isLiteral()) {
            if (STy->getStructName().startswith("struct.anon.") ||
              ...
              if (Id.empty()) new_id = getStructId(STy, M, i);
            } else {
...
bool CallGraphPass::doInitialization(Module *M) {
  ...
  for (GlobalVariable &G : M->globals()) {
    if (G.hasInitializer()) processInitializers(M, G.getInitializer(), &G, "");
  }

  for (Function &F : *M) {
    // Lewis: we don't give a shit to functions in .init.text
    if (F.hasSection() && F.getSection().str() == ".init.text") continue;
    // collect address-taken functions
    if (F.hasAddressTaken()) Ctx->AddressTakenFuncs.insert(&F);
  }
...
bool CallGraphPass::doModulePass(Module *M) {
    ...
    if (GV.getName() == "selinux_hooks") {
      if (ConstantStruct *CS = dyn_cast<ConstantStruct>(GV.getInitializer())) {
        for (int i = 0; i < CS->getNumOperands(); ++i) {
          ...
          if (ConstantStruct *InnerStruct =
                  dyn_cast<ConstantStruct>(ThirdMember)) {
  ...
  while (Changed) {
    Changed = false;
    for (Function &F : *M) Changed |= runOnFunction(&F);
    ret |= Changed;
  }
  return ret;
}
