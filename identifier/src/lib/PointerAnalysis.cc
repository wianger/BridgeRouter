...
void *PointerAnalysisPass::getSourcePointer(Value *P) {
  ...
  while (!EI.empty()) {
    ...
    // Possible sources
    if (isa<Argument>(TI) || isa<AllocaInst>(TI) || isa<CallInst>(TI) ||
...
static void addToSet(std::set<Value *> &addrSet, Value *addr) {
  if (addrSet.find(addr) == addrSet.end()) addrSet.insert(addr);
}
...
void PointerAnalysisPass::detectAliasPointers(Function *F, AAResults &AAR,
  ...
  for (inst_iterator i = inst_begin(F), ei = inst_end(F); i != ei; ++i) {
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(I)) {
      ...
      for (unsigned j = 0, ej = CI->arg_size(); j < ej; ++j) {
  ...
  for (Argument &A : F->args())
    if (A.getType()->isPointerTy()) addr1Set.insert(&A);

  for (Instruction &I : instructions(*F))
    if (I.getType()->isPointerTy()) addr1Set.insert(&I);
  ...
  for (auto Addr1 : addr1Set) {
    for (auto Addr2 : addr1Set) {
      ...
      if (AResult == AliasResult::MustAlias ||
          AResult == AliasResult::PartialAlias) {
        notAlias = false;
