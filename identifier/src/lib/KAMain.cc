...
cl::opt<bool> DumpGraph(
    "dump-graph", cl::desc("Dump the whole relation graph as graph.json"),
    cl::NotHidden, cl::init(false));
...
int main(int argc, char **argv) {
    ...
    if (DumpGraph)
        CAPass.dumpRelationGraph();
    
    // Free LLVMContext objects
    for (auto &pair : GlobalCtx.Modules)
        delete &pair.first->getContext();
